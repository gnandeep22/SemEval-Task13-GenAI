...
Usage:
    Training:   python train_subtask_a.py --train
    Testing:    python train_subtask_a.py --test
    Validation: python train_subtask_a.py --validate
    Prediction: python train_subtask_a.py --predict
    Auto:       python train_subtask_a.py (trains if no model exists, then predicts)
"""
...
INPUT_FILE = r"C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_A\task_a_training_set_1.parquet"
TEST_FILE = r"C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_A\task_a_test_set_sample.parquet"
VALIDATION_FILE = r"C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_A\task_a_validation_set.parquet"
MODEL_OUTPUT = r"C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_A\binary_code_model.pkl"
...
def extract_features(code: str) -> np.ndarray:
    """Extract 40 statistical and structural features from code snippet for binary classification."""
    ...
    # WHITESPACE & FORMATTING (4 features)
    ...
    # TYPE HINTS & DOCUMENTATION (3 features)
    ...
    # INDENTATION CONSISTENCY (2 features)
    ...
    # OPERATORS & SYMBOLS (4 features)
    features.append(len(re.findall(r'===|!==|==|!=|<=|>=', code)))
    features.append(len(re.findall(r'\{', code)))  # Count only opening braces
    features.append(len(re.findall(r'\(', code)))  # Count only opening parens
    features.append(code.count(';') / max(len(lines), 1))
    
    # CODE QUALITY INDICATORS (3 features)
...
def train_model() -> bool:
    ...
    print("=" * 70)
    print("TRAINING MODE - BINARY CODE DETECTION (SUBTASK A)")
    print("=" * 70)
    ...
    try:
        ...
        feature_names = [
            "Total lines", "Total chars", "Avg chars/line", "Max line length", "Non-empty ratio",
            "Indented lines ratio", "Uses tabs", "Trailing whitespace", "Space density",
            "Comment count", "Comment density", "Has structured docs", "Docstring count", "TODO markers",
            "camelCase count", "snake_case count", "Single letter vars", "Long var names", "PascalCase count",
            "Has type hints", "Return type hints", "Decorators/annotations",
            "Function/class defs", "Has error handling", "Has logging", "Import statements", "Return statements",
            "Max nesting", "If/while/for count", "Else/elif count", "Break/continue",
            "Consistent indent", "Indent variance",
            "Equality checks", "Brace count", "Paren count", "Semicolons/line",
            "Constants/acronyms", "Functional programming", "Large uncommented"
        ]
...
def test_model() -> bool:
    ...
    print("=" * 70)
    print("TESTING MODE - BINARY CODE DETECTION (SUBTASK A)")
    print("=" * 70)
    
    # Check if model exists
    if not os.path.exists(MODEL_OUTPUT):
        print(f"\n✗ Error: Model file not found: {MODEL_OUTPUT}")
        print("Please train the model first: python train_subtask_a.py --train")
        return False
...
def validate_model() -> bool:
    ...
    print("=" * 70)
    print("VALIDATION MODE - BINARY CODE DETECTION (SUBTASK A)")
    print("=" * 70)
...
def classify_code(code: str) -> Optional[Tuple[str, np.ndarray]]:
    ...
    if not os.path.exists(MODEL_OUTPUT):
        print(f"\n✗ Error: Model file not found: {MODEL_OUTPUT}")
        print("Please run training mode first: python train_subtask_a.py --train")
        return None
...
def predict_mode():
    ...
    print("=" * 70)
    print("PREDICTION MODE - BINARY CODE DETECTION (SUBTASK A)")
    print("=" * 70)
...
def main():
    ...
    print("\n" + "=" * 70)
    print("BINARY CODE DETECTOR - SUBTASK A")
    print("=" * 70)
    print(f"Version: 1.0")
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--train':
            ...
            print("\nUsage:")
            print("  Training:    python train_subtask_a.py --train")
            print("  Testing:     python train_subtask_a.py --test")
            print("  Validation:  python train_subtask_a.py --validate")
            print("  Prediction:  python train_subtask_a.py --predict")
            print("  Auto mode:   python train_subtask_a.py")
            print("  Help:        python train_subtask_a.py --help")
            print("\nFiles:")

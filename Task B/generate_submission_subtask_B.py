"""
Competition Submission Generator for Authorship Detection (Subtask B)
======================================================================
Generates predictions in CSV format required by scorer.py

Usage:
    python generate_submission_subtask_b.py --input task_b_test.parquet --output submission_subtask_b.csv
    python generate_submission_subtask_b.py --input task_b_validation.parquet --output validation_predictions.csv
"""
...
MODEL_PATH = r'C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_B\authorship_model.pkl'

# Default input/output paths
DEFAULT_TEST_INPUT = r"C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_B\task_b_test_set_sample.parquet"
DEFAULT_VALIDATION_INPUT = r"C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_B\task_b_validation_set.parquet"
DEFAULT_OUTPUT = r'C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_B\submission_subtask_b.csv'
DEFAULT_GOLD_OUTPUT = r'C:\Users\gnand\OneDrive\Desktop\project\SUB_TASK_B\gold_subtask_b.csv'
...
def extract_features(code: str) -> np.ndarray:
    """Extract 50 features from code snippet (must match training)"""
...
def generate_predictions(input_file, output_file, model_path):
    ...
    print("="*70)
    print("COMPETITION SUBMISSION GENERATOR - SUBTASK B")
    print("="*70)
    ...
    try:
        ...
        for label_name, count in submission['prediction'].value_counts().items():
            ...
            print(f"   {label_name:15} {count:6,} ({percentage:5.2f}%) {bar}")
...
def create_gold_labels(input_file, output_file):
    ...
    """
    print("="*70)
    print("GOLD LABELS GENERATOR - SUBTASK B")
    print("="*70)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate competition submission file for Authorship Detection (Subtask B)"
    )
    
    parser.add_argument(
        '--input',
        default=DEFAULT_TEST_INPUT,
        help='Path to input parquet file (e.g., task_b_test.parquet)'
    )
    
    parser.add_argument(
        '--output',
        default=DEFAULT_OUTPUT,
        help='Path to output CSV file (default: submission_subtask_b.csv)'
    )
    ...
    parser.add_argument(
        ...
        help='Path to gold labels CSV file (default: gold_subtask_b.csv)'
    )
